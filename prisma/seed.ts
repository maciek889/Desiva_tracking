...
async function main() {
  ...
  // --- Users ---
  const usersData = [
    { id: "u1", login: "Admin", password: "Desiva2025!", role: "Admin", hourlyRate: 0 },
    { id: "u2", login: "Biuro", password: "Biuro1", role: "Office", hourlyRate: 0 },
    { id: "u3", login: "Marta", password: "MAZA", role: "Worker", hourlyRate: 35 },
    { id: "u4", login: "Krzysztof", password: "KRST", role: "Worker", hourlyRate: 40 },
  ];
  for (const u of usersData) {
    const hashed = await bcrypt.hash(u.password, 10);
    await prisma.user.upsert({
      where: { id: u.id },
      update: { login: u.login, password: hashed, role: u.role, hourlyRate: u.hourlyRate },
      create: { id: u.id, login: u.login, password: hashed, role: u.role, hourlyRate: u.hourlyRate },
    });
  }
  console.log("  ✓ Users");
